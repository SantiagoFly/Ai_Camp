...
namespace Camposol.Common.Services
{
    /// <summary>
    /// Data access service
    /// </summary>
    public class HttpService : IHttpService
    {
        ...
        /// Post data 
        /// </summary>
        public async Task<bool> PostAsync(object postData, string uri, bool readBody, JsonSerializerSettings settings = null)
        {
            CreateHttpClient();
            var data = JsonConvert.SerializeObject(postData);
            var contentPost = new StringContent(data, Encoding.UTF8, "application/json");
            ...
            var resultData = result.IsSuccessStatusCode;

            if (readBody)
            {
                var response = await result.Content.ReadAsStringAsync();
                resultData = JsonConvert.DeserializeObject<bool>(response, settings);
            }
